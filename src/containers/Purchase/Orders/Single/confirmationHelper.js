...
export default (formValues, initialValues) => {
  ...
  const modalData = Object.getOwnPropertyNames(orderChanges).map(propName =>
    propName !== 'details' &&
    <p key={ propName } className={ styles.confirmItem }>
      <Icon type="check" />
  ...
  formValues.details.forEach((skuDetail, index) => {
    ...
    if (initialSkuDetail) { // if its not a new item
      ...
      modalData.push(
        <p key={ skuDetail.name } className={ styles.confirmNewItem }>
          <Icon type="check" />
  ...
  formValues.details.filter(sku => sku.deleted === true && sku.new !== true)
    .forEach(skuDetail =>
      modalData.push(
        <p key={ skuDetail.name } className={ styles.confirmDeleteItem }>
          <Icon type="check" />
        Removed
