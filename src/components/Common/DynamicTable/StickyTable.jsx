...
class StickyTable extends Component {
  constructor(props) {
    ...
    this.maxHeight = this.getNodeSize(document.getElementById('main')).height * 0.75; // set max height at 83% of main container
  }
  ...
  onResizeBody = () => {
    this.yWrapper.style.maxHeight = `${this.getNodeSize(document.getElementById('main')).height * 0.75}px`;
  }
  ...
  setRowHeights = () => {
    ...
    if (this.stickyColumnCount) {
      for (r = 0; r < this.rowCount; r += 1) {
        ...
        if (cellToCopy) {
          height = this.getNodeSize(cellToCopy).height;
  ...
  setColumnWidths = () => {
    if (this.state.stickyHeaderCount) {
      for (let c = 0; c < this.state.columnCount; c += 1) {
        ...
        if (cellToCopy) {
          const width = this.getNodeSize(cellToCopy).width;
